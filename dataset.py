...
def load_cleanunet_dataset(data_dir, train_metadata, test_metadata, audio_min_sec, crop_length_sec, batch_size, sample_rate,
                               n_fft, hop_length, win_length, power, augmentations=None, num_gpus=1):
    """
    Get dataloader with distributed sampling
    """
    train_dataset = CleanUNetDataset(data_dir=data_dir, subset='train', train_metadata=train_metadata, test_metadata=test_metadata, 
    ...
    test_dataset = CleanUNetDataset(data_dir=data_dir, subset='test', train_metadata=train_metadata, test_metadata=test_metadata, 
                                     audio_min_sec=audio_min_sec, crop_length_sec=crop_length_sec, sample_rate=sample_rate, 
                                     n_fft=n_fft, hop_length=hop_length, win_length=win_length, power=power, 
                                     augmentations=augmentations)
